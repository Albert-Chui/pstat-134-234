...
The code chunk above shows the summary statistics of the column "height" but seperated by the 2 genders in the dataset

```{r}
starwars_smaller %>% group_by(gender) %>% na.omit() %>% summarize(
  count = n(), 
  mean = mean(height),
  std = sd(height), 
  min = min(height), 
  "25%" = quantile(height, 0.25),
  "50%" = quantile(height, 0.25),
  "75%" = quantile(height, 0.25),
  max = max(height)
  )

```

It looks kinda worse even though it has the same information.
...
Ignoring the grouping, it looks like a normal distribtion.

1.  Can you identify the character(s) in the data set that have a missing value for gender identity? Which characters are they?

I don't know which characters would be missing from domain knowledge.

```{r}
which(is.na(starwars_smaller$gender))
```

```{r}
starwars_smaller[18,]$name
starwars_smaller[58,]$name
starwars_smaller[59,]$name
starwars_smaller[80,]$name
```

Now I have found that the 4 missing are \[1\] "Jek Tono Porkins" \[1\] "Gregar Typho" \[1\] "Cordé" \[1\] "Sly Moore"

1.  Can you determine why the code in R included the density curve and histogram for those characters while the code in Python did not?

we used geom_density
